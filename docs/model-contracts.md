...
The package names the already-extracted copy in `target_identity` (title, ISBN, YOLO or iOS crop when present) so a crowded shelf frame is scored as that one copy, not as an unlabeled stack. Up to two stored JPEG/PNG references are attached as provider image inputs, with a per-copy YOLO spine crop preferred when it exists. Raw provider responses are saved before normalized assessments. Forbidden keys (geometry, ISBN, merge, money) are dropped before validation so they cannot enter the assessment record.
