...
The package names the already-extracted copy in `target_identity` (title, ISBN, crop when present) so a crowded shelf frame is scored as that one copy, not as an unlabeled stack. Up to two stored JPEG/PNG references are attached as provider image inputs. Prefer the Apple Vision crop (`shelf_scans/crops/…`) for that slot; an optional YOLO 11x-seg mask is the second image when it exists, or the fallback when Vision has no crop. YOLO does not replace Pass B count. Moondream2 is not used. Raw provider responses are saved before normalized assessments. Forbidden keys (geometry, ISBN, merge, money) are dropped before validation so they cannot enter the assessment record.
