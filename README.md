...
## Shelf photos with a language model

Naming books from a still photo, then pricing them in the same call, is on [`experiment/llm-based-scan`](https://github.com/harshsinha-12/library-estimate/tree/experiment/llm-based-scan). RoomPlan still builds the floor plan. The shelf step is not a timer: you take one photo, or attach photos already on the phone. After seal, each JPEG is one model request. That request estimates how many books are visible, names the titles it can read, counts copies in that photo, and uses web search for a current physical paperback or hardcover price. The shelf total is count times unit price for the titles that got a price. Photographing the same books again counts them again.

Fable, Astra Extra, and Jev are not called on that path. A report that marks them `not_run` is that skip. The home scan from 2026-09-24 (Bareilly, 42 copies, 20 titles, 14 priced, 53,118 INR of contents) is on that branch: [library-survey-BC4104BB-F5FD-4A78-A274-72FB69B68E6F.pdf](https://github.com/harshsinha-12/library-estimate/blob/experiment/llm-based-scan/docs/library-survey-BC4104BB-F5FD-4A78-A274-72FB69B68E6F.pdf). How to run it is in that branch’s README.
