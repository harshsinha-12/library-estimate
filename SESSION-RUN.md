...
## 2026-09-21 16:20 IST — Stage 4 pricing, object costs, Bing page parse

Stage 4 backend and iOS pricing screens are in. The physical-device 8–10 book row gate is still open. Stages 1–3 were not reopened. Stage 5 was not started. No production API process was left running.

- **Bing parse.** Bing HTML search yields listing URLs. At most 5 URLs are fetched per search (ISBN or name). The first 10,000 characters of each listing page go to `gpt-5.6-luna` with a strict JSON schema for citations: title, URL, snippet, offer_type, amount, currency, format, condition. Those rows are drafts only. If the model is unconfigured, a local HTML fallback still extracts citations from Bing snippets plus listing excerpts. Cache key remains `sha256(q + market)` with a 12-hour TTL.
- **Row fixture (not a phone sweep).** Sealed 9-copy `row_01` with reverse-sweep of four spines: detected/actual **9/9**; reverse did not duplicate; `row_02` stays partial with recapture. Two same-ISBN copies stayed two IDs. Invalid ISBN `9780143127551` was not used as a price query. Unread slots 4 and 8 stayed visible and unpriced. ISBN search and name-only search both produced Bing drafts; confirming one shared-edition copy did not price its sibling. eBook drafts cannot be confirmed as physical prices. Mug stayed excluded. Building reconstruction used `demo_rebuild_rates_v1` × floor area, basis `replacement_cost`. Ledger recorded `bing_html_search` lines under a $50 cap.
- **Object tagging.** A portrait/AC/table can be pointed, photographed, and given a spoken or typed cost (“this portrait cost us $500”, “this is an AC, this cost us 25000 rupees”). That becomes a draft observation tied to the image, not a confirmed price until review.
- **APIs/UI.** `GET /v1/surveys/{id}/overview`, `POST /v1/assets/{id}/price-search`, `POST /v1/surveys/{id}/price-search-queue`, `POST /v1/assets/{id}/price-observations`. iOS Overview, Inventory row roster, and Price Evidence screens are present. Live shelf sweep now clusters spine outlines on a row and records a manual actual count.
- **Verification.** `make check`: 31 tests, Ruff, compileall. Fixture priced/eligible numerator was at least 3 after confirming ISBN, name, and AC drafts against the eligible set. **Open gate:** no physical iPhone 8–10 book row has been swept, so CHECKPOINTS Stage 4 clock and the added row gate stay unchecked. Record detected/actual and priced/eligible from the phone before closing it.

## 2026-09-21 16:30 IST — listing URL cap and gpt-5.6-luna

Bing search still finds citations; at most **5 listing URLs are fetched per search** (ISBN or name, i.e. per book query). Each fetched page is truncated to 10,000 characters and parsed by `gpt-5.6-luna` against the citation JSON schema. The 7-result unit fixture fetched only the first five URLs. `OPENAI_SMALL_MODEL=gpt-5.6-luna` is set in `.env.local`; credentials were not printed. `make check` still 31 passed. No production API process was started. The physical 8–10 book row gate remains open.

## 2026-09-21 — Stage 5 backend foundation

- Added sealed per-copy evidence replay for Fable (`claude-fable-5-1`) and Astra (`gpt-6-astra`), independent provider calls, raw and normalized response artifacts, Jev (`jev-latest`) typed route proposal, and deterministic human-review vetoes for disagreement, category conflict, low confidence, and appraisal-required assets. No live Fable/Astra/Jev call was made.
- Added Redis per-run provider usage records for OpenAI web search, focused small-model calls, vision title calls, Fable, Astra, and Jev. Records include actual response model, input/output tokens, web-search call count, latency, dated rate source, and estimated USD cost. Unknown models remain unpriced. The pre-call `$50` survey reservation gate stops new provider calls at the cap; this is a conservative spend estimate, not a provider invoice.
- Added an append-only `RLTransition` for every model replay decision, with reward left null pending independent labels. Added model-run, transition, run-usage, survey-usage, JSON-report, and PDF-report APIs. Reports store versions, evidence and listing references, valuation status, provider spend, and limitations.
- Verification: Ruff, compileall, and 45 isolated backend tests pass with ReportLab installed in temporary storage, including an HTTP run-header-to-Redis round trip. The live Redis/R2 storage test passed outside the network sandbox. A sample report PDF was rendered and visually inspected. The remaining full-suite failure is an iOS assertion being edited by another agent (`maximumAspectRatio = 1.05`). The physical 8–10 book phone gate, live provider behavior, auth/security, offline RL training, holdout metrics, and full demo remain open; Stage 5 clock gate is unchecked.

## 2026-09-21 22:41 IST — Stage 5 offline, security, and review paths

- Added independent transition labels, the §13 reward calculation, a replay-backed offline action model, specialist heads, disjoint train/holdout survey IDs, a policy registry, and shadow scoring with explicit action support and overlap warning. The live router still uses its fixed policy; synthetic labels exercised the trainer, with no claim of physical-zone accuracy. Human Stage 3 review decisions now append transitions and are excluded from policy fitting.
- Added opt-in bearer authentication, AES-GCM object encryption, authenticated evidence reads, bounded access logs without token/query/UUID data, survey deletion including create idempotency records, and a dry-run-first retention script. The iOS operator token is stored in Keychain and is sent only to HTTPS URLs. Existing local HTTP development setup is still unauthenticated; TLS deployment, encryption migration, phone package encryption, and redaction remain open.
- Added iOS Processing and Report/PDF views, per-copy capture-evidence links, and the backend observation-to-file mapping that feeds them. Added a labeled-zone evaluator that compares an independently recorded roster to a saved report and emits case-level and grouped numerators/denominators. No real zone label file has been supplied or evaluated yet.
- Verification: 51 isolated backend tests passed; Ruff and `git diff --check` passed. A generic iOS Simulator build passed after Xcode project regeneration. The full backend suite remains 51 passed, one failed: `test_ios_stage_one_permissions_and_sources_exist` expects `maximumAspectRatio = 1.05` in `LiveQualityAnalyzer.swift`, which another agent is editing. No Fable/Astra/Jev live call was made and no physical-device demo was run. Stage 4 row and Stage 5 clock gates remain open.

## 2026-09-21 — Leftover #1 Camera: RoomPlan owns the session (code)

Sequential camera ownership is now productized in the iOS capture flow. No physical device was used.

- One `CameraSessionCoordinator` lock: RoomPlan, then shelf `ARView`, then Pass C `UIImagePicker`. Pass C cannot open the hardware camera while a geometry session is running. RoomPass no longer attaches the RoomPlan `ARSession` into Pass B, and Pass B no longer constructs a fallback `ARSession()`.
- `ARView.automaticallyConfiguredSession` is off. Both camera views pause and release on dismantle. Close-ups are JPEG crops or a later still; Pass A copy does not promise optical zoom.
...
## 2026-09-22 — Leftover #5 inventory, evidence, operator product (code)

- The overview now joins each copy to its count observations, crop paths, shelf-face position, damage close-ups, bound spoken notes, open review tasks, confirmed physical price citations, and operator-placed versus unregistered shelf status. Row detail exposes actual/detected, coverage, partial count interval, and a selectable slot roster. Slots are schematic markers, not claimed image bounding boxes.
...
- Security code/readiness: sealed iOS packages now use Data Protection and backup exclusion. A configurable Vision face-redaction pass covers package JPEGs, records `privacy/face-redaction.json`, and fails sealing rather than silently retaining unredacted bytes when required. R2 plaintext encryption migration and retention deletion now require the exact currently reviewed dry-run plan ID. `deploy/Caddyfile` and `docs/security-deployment.md` define the HTTPS/private-backend run.
- Accessibility code: capture, shelf, exception, processing, inventory, evidence, report, and review flows now use scaled minimum touch targets, icon-plus-text status, VoiceOver semantics/hints/announcements, Dynamic Type-safe layout, and honest camera/AR/3D fallback descriptions. TTS prompts display exact transcripts and disclose AI-generated speech.
